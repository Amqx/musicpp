...
string ImgurApi::UploadImage(const vector<uint8_t> &gif) const {
    if (logger_) {
        logger_->info("Performing Imgur upload with size: {} kB", gif.size() / 1024);
    }
    ...
    curl_mimepart *part = curl_mime_addpart(mime);
    curl_mime_name(part, "image");
    curl_mime_data(part, reinterpret_cast<const char *>(gif.data()), gif.size());
    curl_easy_setopt(curl, CURLOPT_MIMEPOST, mime);
