...
std::chrono::nanoseconds TrackTiming::total() const {
    if (_end < _start) return std::chrono::nanoseconds(0);
    return _end - _start;
}
...
std::ostream & operator<<(std::ostream &os, const TrackTiming &timing) {
    ...
    os << fmtUnix(timing.start()) << " - " << fmtUnix(timing.end()) << " | " << fmt(timing.current()) << " elapsed, "
...
std::ostream & operator<<(std::ostream &os, const Track &track) {
    os << "Track { identity: " << track.identity << ", timing: " << track.timing << ", status: " << track.status << " }";
    return os;
}
